...
const ActiveSchedule = () => {
  ...
  useEffect(() => {
    async function fetchSchedule() {
      ...
      const elapsedTime = moment().diff(moment(schedule.activationTime), "seconds")
  ...
  const completeSchedule = () => {
    axios.patch("/api/schedule", {...schedule, status: "complete"})
 ...
 const hasSchedule = schedule && schedule.id
  return(
      <div className={activescheduleDes.container}>
        <h1 className={activescheduleDes.title} > {hasSchedule ? schedule.title : 'No active schedule to display yet!'} </h1>
        <div className={activescheduleDes.seconds}>
          {
            hasSchedule &&
            (
              secs > 0 ?  <h2 className={activescheduleDes.secview}> {secs} </h2> :  <h2><button className="button is-success"
              onClick={completeSchedule}> Click and Done! </button></h2>
            )
          } 
        </div>
        {
          hasSchedule ? <Link href={`/schedule/${schedule.id}`}>
          <a>
            Go to schedule
          </a>
          </Link> : <Link href={"/"}>
